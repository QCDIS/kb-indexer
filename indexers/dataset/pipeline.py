...
SeaDataNet_EDMED__MetadataRecordsFileName = (
    "SeaDataNet-EDMED-metadata-records.json")
...
def getDatasetRecords__SeaDataNet_EDMED():
    ...
    indexFile = open(
        MetaDataRecordPath + SeaDataNet_EDMED__MetadataRecordsFileName, "w+"
        )
...
def getDatasetRecords__SeaDataNet_CDI():
    with urllib.request.urlopen(
        ...
        data = f.read().decode('utf-8')
    indexFile = open(
        MetaDataRecordPath + SeaDataNet_CDI__MetadataRecordsFileName, "w+"
        )
...
def getOnlineDatasetRecords__LifeWatch(rnd, genRnd, startingPoint):
    indexFile = os.path.join(
        MetaDataRecordPath + Lifewatch__MetadataRecordsFileName
        )
    cnt = 1
    c = 0
...
# {"EDMEDRecord": {"type": "uri", "value":
# "https://edmed.seadatanet.org/report/6325/"}, "Title": {"type": "literal",
# "xml:lang": "en", "value": "Water Framework Directive (WFD) Greece (
# 2012-2015)"}}


def getOnlineDatasetRecords__SeaDataNet_EDMED(rnd, genRnd, startingPoint):
    indexFile = open(
        MetaDataRecordPath + SeaDataNet_EDMED__MetadataRecordsFileName, "r"
        )
    dataset_json = json.loads(r'' + indexFile.read())
    cnt = 1
    random_selection = random.sample(
...
def getOnlineDatasetRecords__SeaDataNet_CDI(rnd, genRnd, startingPoint):
    tree = ET.parse(
        MetaDataRecordPath + SeaDataNet_CDI__MetadataRecordsFileName
        )
...
def clean(doc):
    ...
    stop_free = " ".join(
        [i for i in integer_free.lower().split() if i not in stop]
        )
    punc_free = ''.join(ch for ch in stop_free if ch not in exclude)
    normalized = " ".join(
        lemma.lemmatize(word) for word in punc_free.split() if
        len(word) > 2 and EnglishTerm.check(word)
        )
    return normalized
...
def topicMining(dataset_json, RI):
    ...
    if dataset_json != "":
        Jsontext = ""
        if RI == "SeaDataNet_EDMED":
            Jsontext = getContextualText_SeaDataNet_EDMED(dataset_json)
        if RI == "SeaDataNet_CDI":
            Jsontext = getContextualText_SeaDataNet_CDI(dataset_json)
        if RI == "LifeWatch":
            Jsontext = getContextualText_LifeWatch(dataset_json)
        if RI == "ICOS":
            Jsontext = getContextualText_ICOS(dataset_json)
        if not len(Jsontext):
...
def remove_none(obj):
    if isinstance(obj, (list, tuple, set)):
        ...
        return type(obj)(
            (remove_none(k), remove_none(v)) for k, v in obj.items() if
                ...
                for key in haystack.keys():
                    ...
                    if result:
                        for k, v in result.items():
                            ...
                            foundResults.append(
                                v
                                ) if v not in foundResults else foundResults
...
def datasetProcessing_ICOS(datasetURL):
            ...
            for metadata_property in metadataStar_object:
                ...
                if metadata_property == "ResearchInfrastructure":
                    ...
                    if not len(topics):
                        topics = topicMining(JSON, "ICOS")
                    result = topics
                    result = pruneExtractedContextualInformation(
                    ...
                    if not len(topics):
                        topics = topicMining(JSON, "ICOS")
                    essentialVariables = getDomainEssentialVariables(
                        domains[0]
                        )
                    result = getSimilarEssentialVariables(
    ...
    indexFile.write("}")
    indexFile.close()
    Precision, Recall, Accuracy, F = metadataRecord_similarity_evaluation(
        indexfname + ".json", 'EssentialVariables',
        ['description', 'keywords', 'abstract'], essentialVariables
        )
    CSVvalue = CSVvalue + str(Precision) + "," + str(Recall) + "," + str(
        Accuracy
        ) + "," + str(F) + ","
    Precision, Recall, Accuracy, F = metadataRecord_similarity_evaluation(
        indexfname + ".json", 'potentialTopics',
        ['description', 'keywords', 'abstract'], lstKeywords.split(",")
        )
    CSVvalue = CSVvalue + str(Precision) + "," + str(Recall) + "," + str(
        Accuracy
        ) + "," + str(F) + ","
...
def datasetProcessing_SeaDataNet_EDMED(datasetURL):
    ...
    indexfname = os.path.join(
        indexFiles_root, "SeaDataNet_EDMED_" + unique_filename
        )
    indexFile = open(indexfname + ".json", "w+")
    logfile = os.path.join(indexFiles_root, "logfile.csv")
    CSVvalue = ""
    ...
    for metadata_property in metadataStar_object:
        ...
        if metadata_property in mapping:
            ...
            if (
                ...
                value = pruneExtractedContextualInformation(
    ...
    Precision, Recall, Accuracy, F = metadataRecord_similarity_evaluation(
        indexfname + ".json", 'potentialTopics',
        ['description', 'keywords', 'abstract'], str(lstKeywords).split(",")
        )
    CSVvalue = CSVvalue + str(Precision) + "," + str(Recall) + "," + str(
        Accuracy
        ) + "," + str(F) + ","
...
def datasetProcessing_SeaDataNet_CDI(datasetURL):
    ...
    unique_filename = str(uuid.uuid4())
    indexfname = os.path.join(
        indexFiles_root, "SeaDataNet_CDI_" + unique_filename
        )
    indexFile = open(indexfname + ".json", "w+")
...
def invertedIndexing(datasetTitle):
    ...
    Hashtablefnames = os.path.join(
        indexFiles_root, "Hashtable_" + datasetTitle + ".csv"
        )
    lstKeywords = []
    lstEssentialVariables = []
    lstPotentialTopics = []
    ...
    for indexFile in lstIndexFileNames:
        ...
        if indexFile_object["url"] != []:
            ...
            url = re.findall(r"(?P<url>https?://\S+)", url)
            if not len(url):
                continue
    ...
    with open(Hashtablefnames, 'w') as f:
        for key in hashtable.keys():
            if str(len(hashtable[key])) not in cntCategory.keys():
                ...
                cntCategory[str(len(hashtable[key]))] = cntCategory[str(
                    len(hashtable[key])
                    )] + 1
        f.write(
            "%s, %s, %s\n" % (
        ...
        for key in hashtable.keys():
            value = str(hashtable[key]).replace("'", "").replace(
                "[", ""
                ).replace(
                "]", ""
                )
...
def datasetProcessing_SeaDataNet_CDI_XML(datasetURL):
    ...
    with urllib.request.urlopen(datasetURL) as f:
        data = f.read().decode('utf-8')
    data = data.replace("xml:", "").replace("eml:", "").replace(
        "namespace:", ""
        ).replace(
        "xmlns:", ""
        ).replace("gmd:", "").replace("gco:", "").replace("sdn:", "").replace(
        "gml:", ""
        ).replace("gts:", "").replace("xlink:", "").replace("\"{", "")
    xml = fromstring(data.encode())
    JSON = json.loads(
        r'' + json.dumps(xmljson.badgerfish.data(xml)), cls=Decoder
        )
    ...
    for orgFields in originalFields:
        for potentialvalue in SetOfValues:
            simScore = get_jaccard_sim(
                potentialvalue.lower(), orgFields.lower()
                )
            if (
                    simScore > CommonSubsetThreshold and potentialvalue not in drivedFields and potentialvalue not in lstPotentialValues):
    ...
    if len(drivedValues) and len(originalValues):
        for subDrivedField in drivedValues:
            for originalField in originalValues:
                ...
                if (
                        simScore > CommonSubsetThreshold and subDrivedField not in lstAcceptedValues):
...
def metadataRecord_similarity_evaluation(
        filename, drivedField, originalFields, SetOfPotentialValues
        ):
    # Turn off
    return 0, 0, 0, 0
    ...
    for originalField in originalFields:
        for subDrivedField in dataset_object[drivedField]:
            for subOriginalField in dataset_object[originalField]:
                simScore = get_jaccard_sim(
                    subDrivedField.lower(), subOriginalField.lower()
                    )
    ...
    FalseNegative = getFalseNegative(
        lstdrivedValues, lstOriginalValues, SetOfPotentialValues
        )

    for PotentialVal in SetOfPotentialValues:
        if PotentialVal not in dataset_object[
            drivedField] and PotentialVal not in FalseNegative:
...
# Create index if not exist with correct settings if index exists, change
# settings
