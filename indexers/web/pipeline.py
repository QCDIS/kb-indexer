...
extensionList = open(
    os.path.join(data_dir, 'data_sources/extensions.json'), "r"
    )
extensionList = json.loads(r'' + extensionList.read())

ResearchInfrastructures = open(
    os.path.join(
        data_dir,
...
def get_all_website_links(url, domain_names):
    ...
    while soup == '':
        try:
            soup = BeautifulSoup(
                requests.get(
                    url, verify=True, timeout=5, headers=HEADERS
                    ).content, "html.parser", from_encoding="iso-8859-1"
                )
            break
        except:
...
def extractFiles(url, html):
    ...
    invalidExtensions = ['com', 'js', 'css', 'php', 'asp', 'aspx', 'html',
                         'htm', 'net', 'org', 'edu',
                         'nl', 'it', 'fr', 'de', 'au']
    invalidSymbols = ['/', '#', '%', '?']
...
def crawl(url, domain_names):
    ...
    if (
            (len(url) < 1024)
            and ('emso.eu/events/' not in url)
            and ("mediawiki.envri.eu/index.php/Special:WhatLinksHere/"
                 not in url)
            and ("mediawiki.envri.eu/index.php/Special:RecentChangesLinked/"
                 not in url)
            ):
...
def strippedText(txt):
    # invalidtext = [
    #     "b3d33ef8a1f43935895d9f0b718ebe394f0e48f90bec8a95c15025dd14088c63",
    #     "window.dataLayer"]
...
def processContents(text):
    ...
    for string in text.splitlines():
        ...
        if txt != '' and txt != ' ' and (not validators.url(txt)) and (
                txt not in pageSentences):
    ...
    for entitiy in doc.ents:
        ...
        topics = [a_tuple[0] for a_tuple in Counter(items).most_common(20) if
                  a_tuple[0].isalnum() and not a_tuple[0].isnumeric()]
